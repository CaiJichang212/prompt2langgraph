...
注意：当前源码的 CLI compile 路径存在一个已知签名不一致问题，详见文末“当前已知问题”。修复后，命令入口如下：
...
- 边使用 `from` / `to` 字段，默认 `kind` 为 `linear`
...
- 当前 `src/prompt2langgraph/cli.py` 的 `compile` 写产物路径仍按旧参数调用 `build_compile_report()`；在修复前，`uv run pt2lg compile ...` 会因为缺少 `compile_id` 和 `timings_ms` 报错。修复时需要同步 CLI、public `compile_workflow()` 和 `runtime.artifacts.compile_workflow_to_artifacts()` 的产物写入路径。
