...
说明：Prompt 只生成简化 JSON plan，不代表 runtime `llm` 节点具备真实执行能力。`builtin.echo_llm` 仍是确定性 mock executor。
...
- `invalid_join_edge.json`：target capability 边界示例，`join` 是 IR 可表达的 edge kind，不是 IR schema 错误；v0.1 LangGraph compiler / runner 会拒绝执行
