...
class SideEffectExecutor:
    ...
    纯审批逻辑层，不持有 invoke_actual 引用。
    通过返回审批信号让 compiler 层决定是否调用实际 executor，
    实现 SideEffectExecutor 与 compiler 内部实现解耦。
    """
    ...
    def __call__(self, inputs: dict[str, Any], params: dict[str, Any]) -> dict[str, Any]:
        if self.allow_side_effects:
            # 直接允许路径：返回信号让 compiler 调用实际 executor
            return {"__side_effect_signal__": "allowed", "inputs": inputs, "params": params}

        if self.security.requires_approval:
            decision = interrupt({
            ...
            if isinstance(decision, dict) and decision.get("decision") == "approved":
                # 审批通过：返回信号让 compiler 调用实际 executor
                return {"__side_effect_signal__": "approved", "inputs": inputs, "params": params}
        ...
        if self.security.idempotency_key:
            # 幂等键路径（P2 增强，P1 仅记录 warning diagnostic）
            # P1 实现：仍需审批，幂等去重逻辑在 P2 补齐
            decision = interrupt({
                "kind": "side_effect_approval",
                "node_id": self.node_id,
                "executor_ref": self.executor_ref,
        ...
        raise ExecutorError(E_SIDE_008, f"side_effect node '{self.node_id}' requires approval or idempotency key")
```

> **设计说明**：`SideEffectExecutor` 返回包含 `__side_effect_signal__` 的 dict 作为审批信号，由 compiler 层的 `_node_wrapper()` 根据信号决定是否调用实际 executor。这避免了在 `SideEffectExecutor` 中持有 `invoke_actual` 闭包引用，使 `SideEffectExecutor` 可独立测试，与 compiler 实现解耦。
...
- [ ] **Step 4: 实现 `SideEffectExecutor`**
  ...
  - `allow_side_effects=True` 返回 `{"__side_effect_signal__": "allowed", ...}` 信号；
  - `requires_approval=True` 时 interrupt 并解析 decision，通过返回 `{"__side_effect_signal__": "approved", ...}` 信号；
  - `idempotency_key` 存在时（P1）仍走审批路径，幂等去重逻辑在 P2 补齐；
  - 无审批且无允许时抛防御性 `ExecutorError(E_SIDE_008, ...)`。

- [ ] **Step 4b: 注册 `builtin.side_effect` executor definition**
  - 在 `registry/builtins.py` 的 `builtin_executor_registry()` 中新增 schema-only definition：
    `ExecutorDefinition(ref="builtin.side_effect", type=ExecutorType.BUILTIN, description="Side effect approval wrapper")`；
  - 该 definition 仅用于 registry 校验通过，实际执行由 `SideEffectExecutor` + compiler dispatch 完成；
  - 确保 `side_effect` 节点的 `executor.ref` 引用 `builtin.side_effect` 时能通过 registry 校验。

- [ ] **Step 5: 在 compiler 中处理 side_effect 审批信号**
  - 在 `_node_wrapper()` 内部识别 `node.kind == "side_effect"`；
  - 实例化 `SideEffectExecutor(node_id=..., executor_ref=..., security=..., allow_side_effects=...)` 并调用；
  - 根据 `SideEffectExecutor` 返回的 `__side_effect_signal__` 决定后续行为：
    - `"allowed"` 或 `"approved"`：调用 `_invoke_executor(executor_ref=node.executor.ref, ...)` 执行实际 executor；
    - 无 `__side_effect_signal__`（即 `side_effect_rejected` 结果）：直接返回拒绝结果；
  - 避免递归再次进入 side_effect 包装。

> **错误路径行为修正**：根据 LangGraph Persistence 文档，当 superstep 中部分节点失败时，已成功完成节点的写入会通过 `checkpoint_writes`（pending writes）持久化，恢复时不重新执行这些节点。但对于 `SideEffectExecutor` 场景，interrupt 和实际执行在**同一节点内**，节点未完成则其写入不会被提交。恢复时节点从头重新执行（包括重新调用 `interrupt()`），需重新获得审批授权。这与开发计划文档 8.4 节的描述一致。
...
- [ ] **Step 7b: 多中断 resume 降级策略**
  - 当 Join + Side Effect 组合场景导致多个 `SideEffectExecutor` 在同一 superstep 并行触发 `interrupt()` 时，需要按 interrupt ID 映射 resume value；
  - 根据 LangGraph Interrupts 文档，恢复多个并行中断使用 dict 映射：`Command(resume={interrupt_id: decision, ...})`；
  - **P1 降级策略**：`run_workflow()` 的 `resume_payload` 当前为简单值。P1 中若检测到多个 pending interrupt，逐一恢复处理（用户按 interrupt 顺序逐个 resume），不实现批量 resume；
  - **P2 增强**：支持 `resume_payload` 为 `{interrupt_id: decision}` 映射格式，一次恢复多个并行 interrupt；
  - **LangGraph API 版本说明**：多中断场景需要 interrupt ID，当前代码使用 v1 风格（`result["__interrupt__"]`）。LangGraph v2 API（`version="v2"`）提供 `GraphOutput.interrupts` 和 interrupt ID。第三期 P1 保持 v1 风格，P2 评估升级到 v2 API 以获取更好的 interrupt ID 支持。
...
class EdgeSpec(BaseModel):
    ...
    def join_source_consistency(self) -> "EdgeSpec":
        ...
        # JOIN 边的 source 字段对 JOIN 无实际路由意义，
        # 设为 join_sources[0] 仅满足 EdgeSpec 的非空 source 约束。
        # 编译器和校验器应使用 join_sources 而非 source 做路由判断。
        if self.source != self.join_sources[0]:
            # 自动修正而非报错，降低迁移负担
            self.source = self.join_sources[0]
        return self
```

JOIN edge 示例（`source` 自动设为 `join_sources[0]`，用户也可显式指定）：
...
> **设计说明**：JOIN 边的 `source` 字段对 JOIN 无实际路由意义（真正的源是 `join_sources` 列表）。model_validator 在 `source != join_sources[0]` 时自动修正而非报错，降低 v0.1 → v0.2 迁移负担。编译器和校验器应始终使用 `join_sources` 做路由判断，不依赖 `source` 字段。
...
- [ ] **Step 3: 写 JOIN 校验测试**
  ...
  - `source` 不在 `join_sources` 中：model_validator 自动修正为 `join_sources[0]`，不报错；
...
- [ ] **Step 3a: 实现 JOIN 迁移提示 diagnostic**
  - 当 `kind=JOIN` 且 `join_sources is None` 时，输出 diagnostic 包含明确的迁移提示：
    ...
    - `hint: "Add 'join_sources: [\"source_node_1\", \"source_node_2\"]' listing all fan-in source nodes. The 'source' field will be auto-set to join_sources[0]."`。
...
- [ ] **Step 4: 实现 `SkillPlanRequest` / `SkillPlanResult`**
  ...
  - **与 `PromptPlanResult` 的对齐**：当前 `PromptPlanResult` 只有 `raw_text`、`plan`、`diagnostics` 三个字段，没有 `workflow_spec`。`SkillPlanResult` 新增 `workflow_spec` 字段后，应同步给 `PromptPlanResult` 增加 `workflow_spec` 字段，保持两个 plan API 的返回类型一致。若决定不给 `PromptPlanResult` 加 `workflow_spec`，则 `plan_skill_to_workflow_spec()` 应直接返回 `WorkflowSpec`（通过 diagnostics 列表传递错误信息），与 `plan_prompt_to_workflow_spec()` 的返回类型对齐。
