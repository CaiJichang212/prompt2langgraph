...
- `_check_target_capabilities()` 位于 `runtime/runner.py`（非 `compiler/langgraph_py.py`），当前仍将 JOIN 视为不支持目标能力。
...
- [ ] **Step 2: 扩展 `run_workflow()` 签名并选择有效 checkpointer**
  ...
  - `checkpointer is not None` 时直接传给 `compile_workflow_to_graph(checkpointer=checkpointer)`；
  - 注入 checkpointer 路径不写入 `_THREAD_CHECKPOINTERS`。

- [ ] **Step 2b: 确认 checkpointer 传递链路完整**
  - `run_workflow()` 中注入的 checkpointer 必须传递给 `compile_workflow_to_graph()` 调用；
  - `compile_workflow_to_graph()` 已有 `checkpointer` 参数（`Any | None = None`），确认 runner 调用时正确传入；
  - 注入 checkpointer 时，`_save_thread_state()` / `_load_thread_state()` 不执行（这些函数依赖 `InMemorySaver` 内部结构，不适用于外部 checkpointer）；
  - 注入 checkpointer 时，不写入 `_THREAD_CHECKPOINTERS` 全局字典，checkpointer 生命周期完全由调用方管理。
...
> **前提依赖**：本任务必须在 **Task 1（checkpointer 注入边界）** 完成后开始。`interrupt()` 依赖 checkpointer 才能工作，`run_workflow()` 必须先接受 `checkpointer` 参数，否则审批中断无法生效。
...
- Modify: `src/prompt2langgraph/registry/builtins.py`（新增 `builtin.side_effect` handler 注册）
- Modify: `src/prompt2langgraph/compiler/langgraph_py.py`（side_effect 节点通过 registry handler 执行，compiler 无需特殊 if-else）
- Modify: `src/prompt2langgraph/runtime/events.py`（`RunInterrupt` 增加 `kind` 字段）
- Modify: `src/prompt2langgraph/runtime/runner.py`（`_extract_interrupt` 提取 `kind` 字段）
...
**设计决策：采用 builtin handler 模式（与 `human_gate` 一致）**

参照现有 `human_gate` 的注册模式——直接在 `builtins.py` 中定义 handler 函数并注册到 `builtin_executor_registry()`——`side_effect` 节点同样注册为 builtin executor，由 handler 内部完成审批 + 执行信号返回。compiler 层的 `_node_wrapper()` 不需要对 `side_effect` 节点做特殊 if-else，保持 compiler 纯净。

**接口契约：**

```python
# registry/side_effect_executor.py
class SideEffectApprovalResult:
    """Side Effect 审批结果封装。

    不直接调用实际 executor，仅返回审批信号让调用方（compiler handler）决定后续行为。
    这避免了 SideEffectExecutor 持有 executor invoke 引用，使审批逻辑与执行逻辑解耦。
    """

    def __init__(self, *, decision: Literal["allowed", "approved", "rejected"],
    ...
    def is_executable(self) -> bool:
        return self.decision in ("allowed", "approved")
...
def side_effect_handler(
    ...
    *,
    ...
    """Side Effect 审批中断 handler。

    注册为 builtin.side_effect 的执行入口。
    审批通过后返回 {"__side_effect_approved__": True} 信号，由 wrapper 调用实际 executor。
    审批拒绝返回 {"effect_result": "side_effect_rejected", "reason": ...}。

    与 human_gate 的区别：human_gate 直接返回审批结果作为 state 更新；
    side_effect 需要审批通过后再执行实际副作用操作，因此采用信号模式。
    该信号是内部约定 key，不写入 workflow state——wrapper 在识别后剥离。
    """
    if allow_side_effects:
        return {"__side_effect_approved__": True, "inputs": inputs, "params": params}

    effective_security = security or SecurityPolicy()

    if effective_security.requires_approval:
        decision = interrupt({
            ...
            "inputs": inputs,
            "params": params,
            "idempotency_key": effective_security.idempotency_key,
        })
    ...
    if effective_security.idempotency_key:
        # 幂等键路径（P2 增强，P1 仍走审批路径，幂等去重逻辑在 P2 补齐）
        decision = interrupt({
            "kind": "side_effect_approval",
            "node_id": node_id,
            "executor_ref": executor_ref,
    ...
    # 无审批、无幂等键、无全局允许：防御性拒绝
    # （正常情况应在验证阶段被 E_SIDE_008 拦截，此处为运行时兜底）
    from prompt2langgraph.registry.executors import ExecutorError
    from prompt2langgraph.diagnostics.codes import E_SIDE_008
    raise ExecutorError(E_SIDE_008, f"side_effect node '{node_id}' requires approval or idempotency key")
```

> **与 `human_gate` 模式的一致性说明**：
>
> - `human_gate` handler：`interrupt()` → 返回 `{"approval": approval}` → 直接作为 state 更新
> - `side_effect` handler：`interrupt()` → 返回 `{"__side_effect_approved__": True, ...}` → compiler wrapper 识别信号 → 调用实际 executor → 返回 executor 结果作为 state 更新
>
> `__side_effect_approved__` 是内部信号 key，**不会写入 workflow state**。compiler 层的 `_node_wrapper()` 需要识别该信号并剥离，确保实际 executor 的输出才是最终的 state 更新。这保持了 "审批逻辑与执行逻辑解耦" 的目标，同时避免信号泄露到 state 中。

**`builtin.side_effect` 注册（`builtins.py`）：**

```python
def side_effect(inputs: dict[str, Any], params: dict[str, Any]) -> dict[str, Any]:
    """Side Effect 占位 handler。

    实际审批逻辑由 side_effect_handler() 完成（需要 security / allow_side_effects 上下文）。
    此处作为 registry 注册的默认 handler，确保 schema 校验通过。
    compiler 层在 dispatch 时会替换为带上下文的 side_effect_handler。
    """
    return dict(inputs)


# 在 builtin_executor_registry() 中新增：
ExecutorDefinition(
    ref="builtin.side_effect",
    type=ExecutorType.BUILTIN,
    input_schema={},
    output_schema={},
    handler=side_effect,  # 有 handler 确保 registry 校验通过
)
```
...
> **执行前确认**：Task 1 已完成 `run_workflow()` 的 `checkpointer` 参数注入。本任务的测试可通过 `run_workflow(..., checkpointer=InMemorySaver())` 或直接用 `compile_workflow_to_graph()` + `graph.invoke()` 两种方式验证。

- [ ] **Step 1: 注册 `builtin.side_effect` executor definition**
  - 在 `registry/builtins.py` 中：
    1. 新增 `side_effect()` 占位 handler 函数（直接透传 inputs）；
    2. 在 `builtin_executor_registry()` 中注册 `ExecutorDefinition(ref="builtin.side_effect", type=ExecutorType.BUILTIN, ..., handler=side_effect)`；
  - 该 handler 有具体实现（非 None），确保 registry 校验通过；
  - compiler 层在 `_node_wrapper()` 中识别 `node.kind == "side_effect"` 时，使用 `side_effect_handler()` 替代默认 handler。

- [ ] **Step 2: 实现 `side_effect_handler()` 和 `SideEffectApprovalResult`**
  - 在 `registry/side_effect_executor.py` 中定义：
    - `side_effect_handler(inputs, params, *, security, allow_side_effects, node_id, executor_ref)`；
    - `security: SecurityPolicy | None = None`（**处理 None 情况，使用 `security or SecurityPolicy()` 兜底**）；
    - `allow_side_effects=True` 返回 `{"__side_effect_approved__": True, ...}` 信号；
    - `requires_approval=True` 时 `interrupt()` 并解析 decision；
    - `idempotency_key` 存在时（P1）仍走审批路径，幂等去重逻辑在 P2 补齐；
    - 无审批且无允许时抛防御性 `ExecutorError(E_SIDE_008, ...)`。

- [ ] **Step 3: 在 compiler 中处理 side_effect 审批信号**
  - 在 `_node_wrapper()` 的 `invoke_node()` 内部识别 `node.kind == "side_effect"`；
  - 当节点为 side_effect 时，构造 `side_effect_handler` 闭包并调用（传入 `node.security`、`workflow.policies.allow_side_effects`、`node.id`、`node.executor.ref`）；
  - 若 handler 返回包含 `__side_effect_approved__` 的结果：
    - 剥离信号，调用 `_invoke_executor()` 执行实际 executor（`node.executor.ref` 对应的 builtin 或 tool）；
    - 将实际 executor 的输出作为节点的 state 更新；
  - 若 handler 返回 `side_effect_rejected`，直接返回拒绝结果作为 state 更新；
  - 该逻辑在 `_invoke_executor()` 调用之前执行，避免递归再次进入 side_effect 包装。

> **信号剥离说明**：`__side_effect_approved__` 是内部约定 key，**不会出现在最终 state 更新中**。`_node_wrapper()` 在识别该信号后：
> 1. 提取 `inputs` 和 `params`；
> 2. 调用 `_invoke_executor()` 获取实际 executor 输出；
> 3. 将 executor 输出按 `node.outputs` 映射写入 state；
> 4. 信号本身被丢弃，不写入任何 state key。

> **错误路径行为修正**：根据 LangGraph Persistence 文档，当 superstep 中部分节点失败时，已成功完成节点的写入会通过 `checkpoint_writes`（pending writes）持久化，恢复时不重新执行这些节点。但对于 `side_effect` 场景，interrupt 和实际执行在**同一节点内**，节点未完成则其写入不会被提交。恢复时节点从头重新执行（包括重新调用 `interrupt()`），需重新获得审批授权。这与安全原则一致：审批授权不应跨失败重试持久化。

- [ ] **Step 3b: 在 validator 中检查 side_effect 所需的 state key reducer**
  - 若 workflow 包含 `side_effect` 节点：
    - 检查 `state_schema.reducers["__pt2lg_side_effect_records__"]` 是否为 `APPEND`（幂等去重必需）；
    - 若 side_effect 节点配置了 `idempotency_key`，检查 `state_schema.reducers["__pt2lg_side_effect_results__"]` 是否为 `APPEND`；
    - 未声明时输出 warning diagnostic（不阻断），附带建议补齐声明；
  - 此检查放在 `validate/security.py` 的 `check_security()` 函数末尾。

- [ ] **Step 3c: 记录 side_effect 执行事件**
  - 审批通过后的实际 executor 调用若无异常，通过 `metrics_sink` 记录 `ExternalCallRecord`（status="succeeded"），与 LLM executor 记录风格一致；
...
- [ ] **Step 3d: 扩展 `RunInterrupt` 事件类型**
  - 在 `runtime/events.py` 的 `RunInterrupt` 中增加 `kind` 字段：
    ```python
    ...
    ```
  - 在 `runtime/runner.py` 的 `_extract_interrupt()` 中提取 `kind`：
    ```python
    interrupt_value = interrupts[0].value
    kind = interrupt_value.get("kind", "human_gate") if isinstance(interrupt_value, dict) else "human_gate"
    # ...
    return RunInterrupt(node_id=node_id, kind=kind, payload=payload)
    ```
  - CLI 根据 `kind` 渲染不同提示文案：
    - `human_gate`："Workflow is waiting for human approval..."
    - `side_effect_approval`："Workflow is waiting for side effect approval..."

- [ ] **Step 4: 写审批中断失败测试**
  - 新建 `tests/test_side_effect_executor.py`；
  - 构造 `requires_approval=True` 且 `allow_side_effects=False` 的 side_effect workflow（fixture: `side_effect_requires_approval.json`）；
  - 注入 `InMemorySaver`，首次运行断言 `RunResult.status == "waiting"`，`RunInterrupt.kind == "side_effect_approval"`。

- [ ] **Step 5: 写 approved resume 测试**
...
- [ ] **Step 8: CLI resume 回归**
  - 在 `tests/test_cli.py` 增加 side_effect waiting + resume approved/rejected 覆盖；
  - `pt2lg resume` 继续使用现有 `--resume` JSON 字符串；
  - `RunInterrupt.kind` 用于 CLI 渲染不同的提示文案，resume payload 对两种中断使用相同格式。

- [ ] **Step 8b: 多中断 resume 降级策略**
  - 当 Join + Side Effect 组合场景导致多个 `side_effect` 节点在同一 superstep 并行触发 `interrupt()` 时，需要按 interrupt ID 映射 resume value；
  - **LangGraph v1 API 下多中断限制**：当前项目使用 v1 API（`graph.invoke(input, config)` 无 `version` 参数），`result["__interrupt__"]` 中的 `Interrupt` 对象在 v1 下不暴露 `id` 字段，只有 `value`。这意味着无法通过 `interrupt_id` 精确配对 resume。
  - **P1 降级策略**：`run_workflow()` 的 `resume_payload` 为简单值。P1 中若检测到多个 pending interrupt，逐一恢复处理——用户按 interrupt 顺序逐个调用 `resume`，每次传入 `{"decision":"approved"}` 或 `{"decision":"rejected"}`，直到所有中断被处理或遇到拒绝；
  - **P2 增强**：升级至 LangGraph v2 API（`version="v2"`），利用 `Interrupt.id` 字段支持 `{interrupt_id: decision}` 映射格式，一次恢复多个并行 interrupt；
  - **LangGraph API 版本锁定**：第三期全程使用 LangGraph v1 API。所有 `graph.invoke()` 调用均不显式传入 `version="v2"`，`_extract_interrupt()` 继续读取 `result["__interrupt__"]`。
...
class EdgeSpec(BaseModel):
    ...
    def check_join_sources_for_join_edges(self) -> "EdgeSpec":
        """校验 JOIN 边必须声明 join_sources，不自动修正 source。

        根据 LangGraph 官方文档，并行 superstep 的写入顺序不可预期。
        join_sources 必须由用户显式声明，compiler 层基于此构建 reducer 逻辑。

        不在此处自动修正 source 字段，因为：
        1. Pydantic v2 model_validator 自动修改 self 的行为在不同版本间不一致；
        2. 自动修正会让问题变得隐蔽——用户写错 source 但 validator 修正后看似正确，
           后续 validator 层发现 join_sources=None 仍会报错，造成困惑；
        3. JOIN 边的 source 字段仅用于 Mermaid 渲染和 diagnostic 输出，
           实际 fan-in 语义由 join_sources 列表决定。
        """
        if self.kind != EdgeKind.JOIN:
            return self
        # join_sources 的 None/空/长度 校验留待 validator 层输出结构化 error
        # 此处仅做基本的类型检查
        return self
```

JOIN edge 示例（source 字段仅为 Mermaid 渲染占位，实际 fan-in 语义由 join_sources 决定）：
...
> **设计说明**：JOIN 边的 `source` 字段对 JOIN 无实际路由意义（真正的源是 `join_sources` 列表）。`model_validator` **不自动修正** source 字段，改为在 validator 层输出 warning diagnostic（W_JOIN_001），当 `source` 不在 `join_sources` 中时提示用户修正建议。编译器和校验器应始终使用 `join_sources` 做路由判断，不依赖 `source` 字段。

**JOIN 校验规则汇总：**

| 错误码 | 触发条件 | 严重性 |
|--------|---------|--------|
| E_JOIN_001 | `kind=JOIN` 但 `join_sources` 为空或缺失 | ERROR |
| E_JOIN_002 | `join_sources` 长度 < 2 | ERROR |
| E_JOIN_003 | `join_sources` 中存在不存在的节点 id | ERROR |
| E_JOIN_004 | `join_sources` 中某源节点**已有**指向同一 target 的 LINEAR/CONDITIONAL 边（重复声明） | ERROR |
| E_JOIN_005 | 同一 target 被多个 JOIN 边引用（语义冲突） | ERROR |
| W_JOIN_001 | `source` 不在 `join_sources` 中（不一致） | WARNING + 诊断建议 |
...
- [ ] **Step 2: 扩展 `EdgeSpec`**
  - 新增 `join_sources` 字段 + `check_join_sources_for_join_edges` model_validator；
  - model_validator **不自动修改** source 字段，仅做基本类型检查；
...
- [ ] **Step 3: 写 JOIN 校验测试**
  - `join_sources=None` 或空列表：error（E_JOIN_001）；
  - `join_sources` 包含未知节点：error（E_JOIN_003）；
  - `join_sources` 长度 < 2：error（E_JOIN_002）；
  - target 出现在 `join_sources` 中：error（归入 E_JOIN_001）；
  - `source` 不在 `join_sources` 中：warning（W_JOIN_001），model_validator **不自动修正**；
  - 多源写同一 state key 但无 reducer：warning；
  - 已存在同源同 target LINEAR 边：error（E_JOIN_004，编译器跳过重复边）。

- [ ] **Step 3a: 实现 JOIN 校验错误 diagnostic**
  - `join_check.py` 输出结构化 error diagnostic：
    - `E_JOIN_001`：`"JOIN edge requires join_sources field (v0.2 migration)"`；
    - `E_JOIN_002`：`"JOIN edge requires at least 2 source nodes in join_sources"`；
    - `E_JOIN_003`：`"join_sources references unknown node '{node_id}'"`；
    - `E_JOIN_004`：`"duplicate edge: source '{source}' already has edge to target '{target}'"`；
    - `E_JOIN_005`：`"multiple JOIN edges target '{target}', semantic conflict"`；
    - `W_JOIN_001`：`"edge '{id}' source '{source}' not in join_sources, suggest setting source to '{join_sources[0]}'"`。

- [ ] **Step 4: 实现 `validate/join_check.py`**
  - 返回 `list[Diagnostic]`；
  - 使用新定义的 `E_JOIN_001`~`E_JOIN_005` 和 `W_JOIN_001` 错误码（优先使用新码而非复用旧码，避免语义混淆）；
...
- [ ] **Step 6: 编译 JOIN 边**
  ...
  - **重复边检测**：在 `add_edge()` 前检查 `tracked_edges`（需新增 set 记录已添加的 `(source, target)` 对），若已存在相同 `(source, target)` 则跳过并输出 warning diagnostic；
  - 将 JOIN 的所有 source 加入 `outgoing_sources`，避免被错误连到 `END`。

- [ ] **Step 7: 更新 target capability 检查**
  - 在 `_check_target_capabilities()`（位于 `runtime/runner.py`，非 `compiler/langgraph_py.py`）中将 `EdgeKind.JOIN` 加入 supported；
...
class SkillPlanResult(BaseModel):
    """与 PromptPlanResult 对齐的 Skill 计划结果。

    不包含 workflow_spec 字段，保持与 PromptPlanResult 一致的设计：
    - raw_text：LLM 返回原始文本（用于诊断和审计）
    - plan：解析后的简化 JSON plan（可能为 None）
    - diagnostics：结构化诊断列表
    """
    raw_text: str
...
def plan_skill_to_workflow_spec(
    ...
    *,
    ...
    """Generate, parse, adapt, and return a WorkflowSpec for a skill directory.

    与 plan_prompt_to_workflow_spec() 返回类型完全对齐：
    - 成功时返回 WorkflowSpec
    - 失败时抛出 AdapterParseError 或 ValidationError，包含结构化 diagnostics
    """
```

**任务间依赖标注**：本 Task（Skill → WorkflowSpec）的 Few-shot 示例 2 中使用了 `"executor": {"ref": "builtin.side_effect"}`。该 executor 在 Task 2 Step 1 中注册。若先实施 Task 4，Few-shot 示例中的 `builtin.side_effect` 引用在 registry 中不存在，会导致 validation 失败。建议实施顺序：先完成 Task 2 的 `builtin.side_effect` 注册，或 Task 4 的 Few-shot 示例暂用 `builtin.identity_transform` 占位。
...
- [ ] **Step 1: 写 fake model 成功转换测试**
  ...
  - 断言返回 `WorkflowSpec`，`validate_workflow(result).ok`。
...
- [ ] **Step 4: 实现 `SkillPlanRequest` / `SkillPlanResult`**
  ...
  - **与 `PromptPlanResult` 的对齐**：`SkillPlanResult` 保持与 `PromptPlanResult` 一致的设计：返回 `raw_text`、`plan`、`diagnostics`，不直接包含 `workflow_spec`。`plan_skill_to_workflow_spec()` 直接返回 `WorkflowSpec`（失败时抛出 `AdapterParseError` 或 `ValidationError`），与 `plan_prompt_to_workflow_spec()` 的返回类型完全对齐。
...
- [ ] **Step 5: 实现 `build_skill_plan_prompt()`**
  ...
  - **新增：state schema 约束提示**：
    - state key 命名规则：必须为合法标识符（字母、数字、下划线，不能以数字开头）；
    - reducer 声明的必要性：fanout 场景的聚合结果、JOIN 场景的多源合并，都必须在 `state_schema.reducers` 中声明对应 key 的 reducer（如 `"results": "append"`）；
    - 保留字列表：`__pt2lg_side_effect_results__`、`__pt2lg_side_effect_records__`、`__interrupt__`、`approval` 为编译器保留 state key，LLM 生成的 plan 不能使用这些 key 作为用户自定义 state key；
  - 提示 `__pt2lg_side_effect_results__`、`__pt2lg_side_effect_records__` 为编译器保留 state key，避免与用户自定义 key 冲突。
...
- [ ] **Step 5a: 实现 Few-shot 示例 prompt 段**
  ...
  **示例 1：简单线性 workflow**
  ...
  ```json
  {
    ...
    "nodes": [
      {"id": "search", "kind": "retriever", "executor": {"ref": "builtin.mock_retriever"}},
      {"id": "analyze", "kind": "llm", "executor": {"ref": "builtin.echo_llm"}},
      {"id": "answer", "kind": "llm", "executor": {"ref": "builtin.echo_llm"}}
    ],
    "edges": [
      {"id": "e1", "source": "search", "target": "analyze", "kind": "linear"},
      {"id": "e2", "source": "analyze", "target": "answer", "kind": "linear"}
    ]
  }
  ```

  **示例 2：含高危步骤的 workflow**

  Skill 步骤描述：
  ```
  1. Process the input data
  2. Write results to output file [HIGH RISK: file write]
  ```

  对应 JSON plan 片段：
  ```json
  {
    "name": "file_write_workflow",
    "inputs": {"data": "string"},
    "outputs": {"result": "string"},
    "nodes": [
      {"id": "process", "kind": "llm", "executor": {"ref": "builtin.echo_llm"}},
      {"id": "approve_write", "kind": "human_gate", "executor": {"ref": "builtin.human_gate"}},
      {"id": "write_file", "kind": "side_effect", "executor": {"ref": "builtin.side_effect"},
       "security": {"requires_approval": true}}
    ],
    "edges": [
      {"id": "e1", "source": "process", "target": "approve_write", "kind": "linear"},
      {"id": "e2", "source": "approve_write", "target": "write_file", "kind": "linear"}
    ],
    "policies": {"allow_side_effects": false}
  }
  ```
...
> **文档同步优先级**：文档更新前置到 P1 核心增强的验收标准中，而非仅作为 Task 7 的后续步骤。文档与代码不同步会导致用户误解能力边界。
