...
### 6.6 优先兼容 OpenAI-style 第三方接口

第一期 Prompt 计划生成推荐采用 `langchain_openai` 作为接入方式，以兼容 OpenAI 风格接口及第三方兼容 API 为目标，但不扩展为完整多供应商框架。
...
- 能兼容 OpenAI 风格接口以及第三方兼容 API；
- 至少具备最小必需的调用配置能力，例如模型标识、API 基础地址、认证信息和基础生成参数；
- 该接入仅用于 `Prompt → JSON plan` 生成，不扩展到 runtime `llm` 节点执行能力；
- 不在第一期内演化为完整 provider 抽象层。

验收重点是：**能稳定接一个 OpenAI-style / 第三方兼容 LLM API 完成计划生成。**
